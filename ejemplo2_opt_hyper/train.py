...
class MLflowCheckpoint(Callback):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        mlflow.keras.log_model(self._best_model, "model", conda_env="conda.yaml")

    def on_epoch_end(self, epoch, logs=None):
        """
        Log Keras metrics with MLflow. If model improved on the validation data, evaluate it on
        a test set and store it as the best model. This a predefined function in Callback class.
        """
        # logs should be passsed by keras
        if not logs: 
            return
