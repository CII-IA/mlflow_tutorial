...
def run(training_data, max_runs, max_p, epochs, metric, seed):
    ...
    def new_eval(nepochs, experiment_id, null_train_loss=_inf, null_val_loss=_inf, null_test_loss=_inf):
        def eval(parms):
            lr, momentum = parms
            with mlflow.start_run(nested=True) as child_run:
                
                # Here we use the Python API to execute the train.py file.
                # The execution is performed in its own thread
